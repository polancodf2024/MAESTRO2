...
# Función para leer archivo CSV remoto directamente
def leer_csv_remoto():
    try:
        ssh = paramiko.SSHClient()
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
...
# Función para enviar convocatoria a todos los correos activos (CORREGIDA)
def enviar_convocatoria_a_activos():
    try:
        # Leer el archivo CSV remoto directamente
        df = leer_csv_remoto()
        if df is None:
            return
        ...
        # La tercera columna (índice 2) es el correo electrónico
        columna_correo = df.columns[2]
        
        # Asumimos que la última columna es el estado
        columna_estado = df.columns[-1]
        
        st.write(f"Usando columna '{columna_correo}' para correos y '{columna_estado}' para estado")

        # Obtener correos activos (ignorando mayúsculas/minúsculas y espacios)
        ...
        for correo in correos_activos:
            if pd.notna(correo) and '@' in str(correo):
                ...
                st.warning(f"Correo inválido omitido: {correo}")
